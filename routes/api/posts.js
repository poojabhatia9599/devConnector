...
// @desc     Create a post
// @access   Private
router.post('/', [authMiddle, [check('text', 'Text is required').not().isEmpty()]], async (req, res) => {
...
// @access   Private
router.post('/comment/:id', [authMiddle, [check('text', 'Text is required').not().isEmpty()]], async (req, res) => {
	const errors = validationResult(req);
	if (!errors.isEmpty()) {
		return res.status(400).json({ errors: errors.array() });
	}
